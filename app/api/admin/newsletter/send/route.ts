...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const subscriber of subscribers) {
      try {
        // Decrypt email
        const decryptedEmail = subscriber.email ? decryptData(subscriber.email) : subscriber.email
        
        if (!decryptedEmail || !decryptedEmail.includes('@')) {
          results.failed++
          results.errors.push(`Invalid email for subscriber`)
          continue
        }

        // Replace variables (use generic name if we can't get it)
        let personalizedBody = emailBody.replace(/{{customerName}}/g, 'Valued Customer')
  ...
  <table role="presentation" style="width: 100%; border-collapse: collapse; background: linear-gradient(135deg, #f3e8ff 0%, #ffffff 50%, #fce7f3 100%);">
    <tr>
      <td align="center" style="padding: 40px 20px;">
        ...
        results.failed++
        results.errors.push(`Error processing subscriber: ${error.message}`)
      }
    }
